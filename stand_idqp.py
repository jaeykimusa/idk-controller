...
class Go2Controller:
    ...
    def update_state(self, q: np.ndarray, qd: np.ndarray, qdd: np.ndarray, u: np.ndarray):
        self.q = q
        self.qd = qd
        self.qdd = qdd
        self.u = u
    ...
    def getJointPDControlWithTrajectory(self, dt: float) -> np.ndarray:
        ...
        # Compute PD control
        u = self.kp * (q_target - q_current) + self.kd * (qd_target - qd_current)
    ...
    def getJointPDControl(self) -> np.ndarray:
        ...
        u = self.kp * (q_desired - q) + self.kd * (qd_desired - qd)
        return u
